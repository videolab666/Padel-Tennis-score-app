...
// Импортируем компонент VmixButton, если его еще нет
      ...
      {matches.map((match) => (
        <Card key={match.id} className="hover:bg-muted/50 transition-colors">
          <CardContent className="p-4">
            ...
            {/* Найдите место, где отображается список матчей, и добавьте кнопку VmixButton
            Например, в функции renderMatch или аналогичной: */}
            <div className="flex space-x-2">
              <Button variant="outline" size="sm" onClick={() => router.push(`/match/${match.id}`)}>
                Открыть
              </Button>
              <VmixButton matchId={match.id} size="sm" directLink={true} />
            </div>
          </CardContent>
        </Card>
      ))}
