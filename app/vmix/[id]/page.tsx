  ...
  useEffect(() => {
    const loadMatch = async () => {
      try {
        ...
        logEvent("info", `vMix страница: начало загрузки матча ${params.id}`, "vmix-page")
        ...
        if (typeof window !== "undefined" && window.localStorage) {
          try {
            // Пробуем получить матч напрямую из localStorage
            const matchKey = `match_${params.id}`
            ...
            if (matchData) {
              ...
              if (matchesList && Array.isArray(matchesList)) {
                const foundMatch = matchesList.find((m) => m.id === params.id)

                if (foundMatch) {
                  logEvent("info", `vMix страница: матч найден в списке матчей`, "vmix-page", { matchId: params.id })
        ...
        if (!matchData) {
          ...
          try {
            matchData = await getMatch(params.id)
            if (matchData) {
              matchSource = "getMatch"
              logEvent("info", `vMix страница: матч найден через getMatch`, "vmix-page", { matchId: params.id })
            }
          } catch (getMatchError) {
